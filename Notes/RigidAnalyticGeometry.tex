...
\newcommand{\QC}[0]{\underline{\mathfrak{Q}}_C}
\newcommand{\ab}[1]{\vert #1 \vert}
\newcommand{\st}[0]{\:\straw\:}
\newcommand{\cGamma}[0]{\clet\Gamma}
\newcommand{\fib}[1]{%
...
\begin{proof}
    ...
    \par If $U$ and $V$ are open subsets such that $U\cap\Lim\U$ and $V\cap\Lim\U$ are not emptyset, there are $\Omega\subseteq U$ and $\Theta\subseteq V$ as above (as $\QC(X)$ is a topology base), hence $U\cap V\cap\Lim\U$ is not empty. Finally by Proposition \ref{proposition_2}, $\Lim\U\not=\emptyset$ as $X$ is quasi-compact.
\end{proof}
...
\end{lemma}
\begin{proof}
    Let $U\in\Nfil_x$. Then there is $\Omega\in \B$ such that $x\in\Omega\subseteq U$. By our assumption, $\Omega\in\U$ hence $U\in\U$ thus $g$ is a limit of $\U$. 
    Let $x\not\in\overline{\{g\}}$ then there is $U\in\Nfil_x$ such that $g\not\in U$. As $\B$ is a topology base there is $\Omega\in\B$ such that $x\in\Omega\subseteq U$. Then $g\not\in \Omega$. Hence $\Omega\in\U$ Since $\Omega$ is a neighbourhood of $x$, this shows $x\not\in\Lim\U$. Thus $g$ is a generic limit. This proves a).
...
\begin{corollary}[Of Proposition \ref{proposition_4}]
    ...
    \end{enumerate}
    In this case, the open subsets $U$ is quasi-compact if and only if it is clopen and when $Y$ is a spectral space a map $Y\to X$ is spectral it is continuous.
\end{corollary}
\begin{proof}
    $X$ has unique generic limits which are the generic limits of ultrafilters and by Corollary \ref{corollary_1}, the open subsets which are invariant under these limits are preimages of the open subsets are also clesed.
    \par To show these two are equivalent. If $U$ is closed, it is quasi-compact as it is a closed subset of a quasi-compact space. If $U$ is quasi-compact then it is closed by Proposition \ref{fact_3}. If $Y\stackrel{f}{\to} X$ is continuous and $\Omega\in\underline{Q}_C(X)$, then $\Omega$ is clopen hence $f^{-1}\Omega$ is clopen in $Y$, hence $f^{-1}\Omega\in \underline{Q}_C(Y)$.
\end{proof}
\begin{definition}
...
\end{definition}
\begin{remark}
    From the basis axioms of boolean algebras. Clearly we have $X\in \Alg$ and $M_1,M_2\in\Alg\Rightarrow M_1\cap M_2\in\Alg$. 
\end{remark}
\begin{proposition}
    \label{proposition_5}
    Let $X$ be a spectral space. The following boolena algebras of subsets of $X$ coincide.
    \begin{enumerate}
        \item The boolean algebra generated by $\underline{Q}_c(X)$.
        \item $\{M\subseteq X\sep \text{$M$ is closed under generic limits of ultrafilters}\}$.
    \end{enumerate}
    Let $\mathcal{K}_x$ be this boolean algebra of subsets of $X$. The topology on $X$ obtained by taking $\mathcal{K}_X$ as a topology base coincides with the one for which preserving the subsets $M\subseteq X$ which are closed under $\glim$ of ultrafilters are closed subsets.
...
\end{proposition}
\begin{proof}[Not Required for the Exam]$\:$
...
\begin{definition}
    ...
    Let $\Spr(R)$ be the set of equivalent classes of valuation of $R$. For $x\in\Spr(R)$, we choose a representative $\vert\cdot\vert_x$.
    Let $\Gamma_x$  be the group generated by those $\vert r\vert_x$ which are non-zero. For $m\in\N$ and $(r_i)_{i=0}^m\in\R^{m+1}$, let 
    \begin{equation*}
        \mathcal{R}(r_0|r_1,\cdots,r_m)=\mathcal{R}_{\Spr(R)}(r_0|r_1,\cdots,r_m) = \{x\in\Spr(R)\sep 0\not=\vert f_0\vert_x\geq\vert f_i\vert_x, 1\leq i\leq m\}.
    \end{equation*}
    These are called rational open subsets. We equip $\Spr(R)$ with the topology for which the set of rational open subsets is a topology base. 
    \par If $A\stackrel{\alpha}{\to}B$ is a ring homomorphism then this induces 
    \begin{equation*}
        \Spr(B)\stackrel{\Spr(\alpha)}{\to}\Spr(A)
    \end{equation*}
    sends $x\in\Spr(B)$ to the equivalence class of the valuation,
    \begin{equation*}
        a\mapsto\vert\alpha(a)\vert_x
    \end{equation*}
    of $A$. We set,
    \begin{equation*}
        \gamma(a) = \Spr(A)\backslash \mathcal{R}(a|).
    \end{equation*}
\end{definition}
\begin{remark}
    The topology there is the coordinated? for which
    \begin{equation*}
        \Spr\stackrel{x\mapsto\supp(x)}{\to}\Spec
    \end{equation*}
    is continuous and all
    \begin{equation*}
        \{x\in\Spr(A)\backslash R(a)\sep \vert\alpha\vert_x\leq\vert a\vert_x\}
    \end{equation*}
    for $(a,\alpha)\in A^2$ are open.
\end{remark}
\begin{remark}
    The set of rational open subset is closed under finite intersections in $\Spr(R)$. Indeed,
    \begin{align*}
        \mathcal{R}(1|) &= \Spr(R),\\
...
\end{remark}
One way of constructing generic limits of ultrafilters in $\Spr(R)$ would be to use Proposition \ref{fact_5_1} and 
\begin{equation*}
    \{(r,s)\in R^2\sep \vert r\vert_g\leq\vert s\vert_g\}\defeq \{(r,s)\in R^2\sep \{x\in\Spr(R)\sep \vert r\vert_x\leq \vert s\vert_x\}\in\U\}.
\end{equation*}
...
\begin{proposition}
    ...
    For every ring $R$, $\Spr(R)$ is a spectral space and a subset of the form $\cR(f_0|f_1,\cdots,f_m)$ with $m\in\N$ and $9(f_i)_{i=0}^m\in R^{m+1}$ is quasi-compact.
    \par The generic limit of the ultrafiler $\U$ on $\Spr R$ is given by the equivalence classof the valuation,
    \begin{equation*}
        R\stackrel{r\mapsto (\text{the $\U$-equivalence class of $(\ab{r}_\gamma)_{\gamma\in\Spr R}$})}{\longrightarrow}\left({\prod_{\gamma\in\Spr R}}_\U\Gamma_\gamma\right)\cup\{0\}=\prod_\U(\Gamma_\gamma\cup\{0\}). \tag{1}\label{proposition_6_1_1}
    \end{equation*}
    Moreover, the map,
    \begin{equation*}
        \Spr(R)\stackrel{\gamma\mapsto \supp\ab{\cdot}_\gamma}{\longrightarrow}\Spec R
    \end{equation*}
    is spectral and for every ring homomorphism $R\stackrel{\rho}{\to}S$, the map 
    \begin{equation*}
        \Spr(S)\stackrel{\Spr(\rho)}{\to}\Spr(R)
    \end{equation*}
    is also spectral.
\end{proposition}
\begin{proof}
    ...
    \end{align*}
    Lemma \ref{lemma_1} can indeed be applied. That the induced map $\Spr(R)\stackrel{\supp}{\to}\Spec(R)$ is spectral follows from 
    \begin{equation*}
        \supp(D(f)) = \cR(f|).
    \end{equation*}
    Proposition \ref{fact_1_2_6}. That $\Spr(\rho)$ is spectral follows immediately from 
    \begin{equation*}
        \Spr(\rho)^{-1}\cR(f_0|f_1,\cdots,f_m) = \cR(\rho(f_0)|\rho(f_1),\cdots,\rho(f_m)).
    \end{equation*}
\end{proof}
...
\begin{proposition}
    \label{fact_6_3}
    For every ideal $I\subseteq R$, if $R\stackrel{\pi}{\to}R/I$ denotes the canonical projection, then the induced map $\Spr(R/I)\stackrel{\pi}{\to} \Spr(R)$ is a homeomorphism. Explicitly, we have,
    \begin{equation}
        \Spr(R/I) = V_{\Spr(R)}(I) \defeq\{x\in\Spr(R)\sep \supp x\supseteq I\} = \Spr(R)\backslash\bigcup_{f\in I}\cR(f).\label{eq_spr_v_formula}
    \end{equation}
    ...
    \begin{equation*}
        \Spr(R/I) = \bigcup_{i=1}^m \cR(g_i)
    \end{equation*}
    which is the embedded subset. Alternatively, we have,
    \begin{equation*}
        V_{\Spr(R)} = \{x\in\Spr(R)\sep \forall f\in I, \ab{f}_x = 0\}.
    \end{equation*}
    Furthermore, we can identify this with a homeomorphism,
    \begin{equation*}
        \Spr(R/I)\underset{\simeq}{\stackrel{\Spr(\pi)}{\to}} V(I).
    \end{equation*}
    \par Let $S$ be a multiplicative subset of $S$ and $\iota:R\to R_S$ be a canonical inclusion then,
    \begin{equation}
        \Spr(R_S) = \underset{\Spr(\iota)}{\stackrel{\simeq}{\to}}\bigcap_{s\in S}(\Spr(R)\backslash V(s)) = \bigcap_{s\in S}\cR(s|).
    \end{equation}
\end{proposition}

\begin{proposition}
    \label{fact_6_4}
    Let $s$ be a specialization of $\eta\in\Spr(R)$, then 
    \begin{equation}
...
\begin{definition}
    ...
    \end{equation*}
    For an equivalence class $x\in\Spr(R)$ where $v\in x$, we may denote,
    \begin{equation*}
...
\begin{proposition}
    \label{fact_6_6}
    Let $s$ be a specialization of $\eta\in\Spr(R)$. We have,
    \begin{equation*}
...
\begin{proposition}
    \label{fact_6_7}
    Let $s$ be a specilization of $\eta\in\Spr(R)$. 
    \begin{equation*}
...
\begin{proposition}
    \label{fact_6_10}
    \begin{equation*}
        \cGamma_v = \{\gamma\in\Gamma_v\sep \exists r\in R \:\st\: \gamma v(r)\geq1\}
    \end{equation*}
\end{proposition}
\begin{remark}
    If $H$ is a convex subgroup of $\Gamma_v$, then $\cGamma_v\subseteq H$. Furthermore, we have,
    \begin{equation*}
        v|_{H}(r)=\begin{cases}
